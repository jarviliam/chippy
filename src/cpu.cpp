...
void CPU::step() {
  ...
  switch (getOpcode(instruction)) {
    std::cout << getOpcode(instruction) << '\n';
  case SYS:
    break;
  case CLS:
    m_screen->clear();
  ...
  case ADD: {
    byte vx = getVxFromAddress(instruction);
    byte target = instruction & ByteMask;
    registers[vx] = registers[vx] + target;
    break;
  }
  case LD_VX: {
    byte vx = getVxFromAddress(instruction);
  ...
  case SHR_VX: {
    byte vx = getVxFromAddress(instruction);
    byte vy = getVyFromAddress(instruction);
    registers[15] = registers[vx] & 0x000F;
    registers[vx] >>= 1;
    break;
  }
  case SUBN: {
  ...
  case SHL: {
    byte vx = getVxFromAddress(instruction);
    byte vy = getVyFromAddress(instruction);
    registers[15] = (registers[vx] & 0xF000);
    registers[vx] <<= 2;
    break;
  }
  case SNE_VX: {
  ...
  case LD_B: {
    byte vx = getVxFromAddress(instruction);
    memory[i_reg] = registers[vx] / 10;
    memory[i_reg + 1] = (registers[vx] / 10) % 10;
    memory[i_reg + 1] = (registers[vx] % 100) % 10;
    break;
  }
  case LD_I_VX: {
    // TODO(tests)
    byte vx = getVxFromAddress(instruction);
    for (int i = 0; i <= registers[vx]; ++i) {
